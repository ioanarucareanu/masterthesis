  ...
  scr = top-down visit (scr) {
    case qrun : call(name(name("mysql_query" )), params): {
      if (![actualParameter(scalar(string(_)), _)] := params && 
          ...
          ![actualParameter(scalar(encapsed(_)), _)] := params && 
    ...
    case qrunVar: call(name(name("mysql_query" )), 
      [actualParameter(var(name(name(varName))), _)]): {
      if (trace.info == queryDescr()) {
    ...
    case qrunVar: call(name(name("mysql_query" )), 
      [actualParameter(var(name(name (varName))), _), _]): {
      if (trace.info == queryDescr()) {
        trace.info = queryDescr(just(qrunVar.parameters[1].expr), qrunVar.parameters[0].expr,
          qrunVar@at.begin.line,getQueryResultVariable(qrunVar)); 
        trace = resetTraceFlags(trace);
        fail scr;
      }
      //TODO
...
public bool isAppendIfCase(int queryLine) { 
  ...
  top-down visit (context) {
    case ifStmt: \if(_, _, _, _): { 
      ...
      //TODO <;=, >;=
      if ((ifStmt@at.begin.line <= queryLine) && (ifStmt@at.end.line >= queryLine)) { 
        return false ;
      }
      return true ; 
    }
    case elseClause: \else(_): { 
      ...
      //TODO <;= >;=
      if ((elseClause@at.begin.line <= queryLine) && (elseClause@at.end.line >= queryLine)) { 
        return false ;
      }
      return true ; 
    }
