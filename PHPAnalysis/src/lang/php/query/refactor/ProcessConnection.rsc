...
public Connection createMockConnection() {
	return connection(var(name(name("dbcnx"))), actualParameter(var(name(name("host"))), false), actualParameter(var(name(name("user"))), false), actualParameter(var(name(name("pass"))), false));
}
...
private tuple[Connection con, System sys] refactorMysqlConnect(System sys) {
	...
	return <createMockConnection(), sys>;
}
